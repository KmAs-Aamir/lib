/* kmascodecolor ver 1.31 by kmasschools.com */
function kmasCodeColor() {
  ...
  for (j = 0; j < k; j++) {
    ...
    l = x.length;
    for (i = 0; i < l; i++) {
      x[i].innerHTML = kmasCodeColorize(x[i].innerHTML, modes[j]);
    }
  }
}
function kmasCodeColorize(x, lang) {
  ...
  function htmlMode(txt) {
    ...
    php = new extract(rest, "&lt;\\?php", "?&gt;", phpMode, "kmasPHPPOS");
    rest = php.rest;
    comment = new extract(rest, "&lt;!--", "--&gt;", commentMode, "kmasHTMLCOMMENTPOS");
    rest = comment.rest;
    ...
    for (i = 0; i < comment.arr.length; i++) {
        rest = rest.replace("kmasHTMLCOMMENTPOS", comment.arr[i]);
    }
    for (i = 0; i < php.arr.length; i++) {
        rest = rest.replace("kmasPHPPOS", php.arr[i]);
    }
    return rest;
  }
  ...
  function cssMode(txt) {
    ...
    comment = new extract(rest, /\/\*/, "*/", commentMode, "kmasCSSCOMMENTPOS");
    rest = comment.rest;
    while (rest.search("{") > -1) {
      s = rest.search("{");
    ...
    rest = rest.replace(/}/g, "<span style=color:" + cssdelimitercolor + ">}</span>");
    for (i = 0; i < comment.arr.length; i++) {
        rest = rest.replace("kmasCSSCOMMENTPOS", comment.arr[i]);
    }
  ...
  function jsMode(txt) {
    ...
    for (i = 0; i < esc.length; i++) {
      rest = rest.replace("kmasJSESCAPE", esc[i]);
    }
